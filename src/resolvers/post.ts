...
export class PostResolver {
  ...
  async posts(
    ...
    let posts = await qb.execute(
      `select * from post ${
        cursor
          ? `where "created_at" < (select to_timestamp(${cursor} / 1000))`
          : ""
      } order by "created_at" DESC limit ${replacements[0]}`
    );

    // if(!id) id = req.session.userId
    posts = (await posts).filter(
      (post: any) => post.author === req.session.userId
    );
  ...
  async followersPost(@Ctx() { em, req }: MyContext): Promise<Post[] | any> {
    const me = await em.findOne(User, { id: req.session.userId });
    const posts = await em.find(Post, { author: me.followers });
    return posts;
  }
  ...
  async deleteSub(
    ...
    if (req.session.userId !== postId.owner.id)
      return {
        errors: [
          {
            field: "post",
            message:
              "Error deleting this comment, you are not the owner of it.",
          },
        ],
      };
  ...
  async updatePost(
    ...
    if (req.session.userId !== post.author)
      return {
        errors: [
          {
            field: "post",
